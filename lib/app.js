...
(function (global) {
  ...
  async function runSend() {
    ...
    try {
      ...
      if (!chainMode) {
        ...
        UI.note('Over ' + U.fmtBytes(app.FOLD_LIMIT) + ': the link will carry a fingerprint of the six… of the parts rather than the whole file, and each part is still checked.');
      }
      ...
      await afterFailure(backend, e, stranded, expMs);
  ...
  async function afterFailure(backend, e, ids, expMs) {
    ...
    if (backend && backend.key === 'selfhost' && typeof backend.remove === 'function') {
      ...
      if (!failed) {
        UI.note(gone + ' of ' + ids.length + ' uploaded part' + (ids.length === 1 ? '' : 's') +
          (gone === 1 ? ' was' : ' were') + ' deleted from your bucket, so nothing is left behind.');
      } else {
        UI.warn(gone + ' of ' + ids.length + ' part' + (ids.length === 1 ? '' : 's') +
          ' were deleted; ' + failed + ' stayed in the bucket. If the bucket does not allow the DELETE' +
    ...
    UI.warn(ids.length + ' part' + (ids.length === 1 ? '' : 's') + ' had already reached ' + shortName(backend) +
      ' and will stay there' + held + '. Nothing can be rebuilt from them without the link, and ' +
      shortName(backend) + ' gives no way to delete them early.');
  }
