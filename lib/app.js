...
(function (global) {
  ...
  function plan(files) {
    ...
    var cap = partCap(backend);
    var total = files.reduce(function (a, f) { return a + f.size; }, 0);
    var parts = files.length === 1 ? MD.pack.planParts(total, cap, 1) : [];
    return { backend: backend, cap: cap, total: total, parts: parts, count: parts.length };
  }
  ...
  async function runSend() {
    ...
    try {
      ...
      if (pw) {
        ...
        var salt = U.randomBytes(16);
        var mat = await MD.crypto.makeEncryptor(pw, salt);
        enc = { key: mat.key, verifier: mat.verifier, ivPrefix: U.randomBytes(4), salt: salt, blockSize: encPlan.blockSize };
      }

      var fingerprint = '';
      if (file.size <= backend.maxPartBytes || backend.key === 'local' || backend.key === 'mockhost') {
        ...
        UI.progress(0, 1, 'Skipping the whole-file fingerprint (' + U.fmtBytes(file.size) + ' is big; each part is still checked individually) …');
      }
      ...
      for (var i = 0; i < parts.length; i++) {
        ...
        if (enc) {
          ...
          blocksTotal += blocks;
          plain = null;
          partSha = await MD.crypto.sha256Hex(cipher);
          ...
          body = MD.backends.partBody(backend, file.slice(p.start, p.start + p.size), fields, file.name, file.type || 'application/octet-stream');
        }
      ...
      // form framing that carried it — so the manifest records the payload size.
        ...
        uploaded += p.size;
        base = res.base || base;
        sent.push({ i: res.id, s: payloadSize, h: partSha.slice(0, 16), b: blocks });
      ...
      if (e && e.cancelled) UI.note('Cancelled. Parts already uploaded stay on the host until they expire.');
  ...
  function planInfoFor(backend, file) {
    ...
    var cap = partCap(backend);
    return {
      parts: MD.pack.planParts(file.size, cap, 1),
      cap: cap,
  ...
  async function runReceive(m) {
    ...
    try {
      ...
      if (!res.verified) {
        throw new Error('Fingerprint mismatch: the reassembled bytes hash to ' + res.fingerprint.slice(0, 12) +
          ' but the sender recorded ' + String(mm.h).slice(0, 12) + '. Do not trust this file.' +
          (writer ? ' It was written to disk anyway so you can inspect it.' : ''));
      }
