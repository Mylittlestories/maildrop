...
(function (global) {
  ...
  var defaults = {
    ...
    mailTo: '',             // last recipient, typed once on this device — see docs/PRIVACY.md
  ...
  function loadCfg() {
    ...
    if (K && typeof K === 'object') {
      ['backend', 'receiveBase', 'partCapBytes', 'expiry', 'mailTo'].forEach(function (k) {
        if (K[k]) c[k] = K[k];
      });
  ...
  // Part size policy. Two things pull against each other, and only one of them is
  // the link: never above what the host accepts, never below 1 MiB (a link with
  // thousands of parts is useless), and a manual override always wins over the
  // recommended 16 MiB floor — the tester who types 1MB means it.
  //
  // The other pull is about failure. A part is what a retry costs, so one 950 MiB
  // request on a phone whose radio drops every minute is a transfer that cannot
  // finish: every attempt dies at the same place and the resume gains nothing. A
  // 12 MiB part at ~1 Mbps is about 90 seconds of exposure, and a completed part is
  // a completed part — the next attempt starts after it. Files that already fit in a
  // couple of requests are not split at all, because there the only effect would be
  // a longer link.
  var MIN_PART = 1 * 1024 * 1024;
  var SOFT_MIN_PART = 16 * 1024 * 1024;
  var ATTEMPT_TARGET = 12 * 1024 * 1024;
  var MAX_PARTS_KEEN = 40;
  function partCap(backend, total) {
    var ceiling = Math.max(SOFT_MIN_PART, backend.maxPartBytes);
    var cap = ceiling;
    var size = Number(total) || 0;
    if (size > ATTEMPT_TARGET * 2) {
      var spread = Math.ceil(size / MAX_PARTS_KEEN);      // …and never more parts than the link can carry
      cap = Math.min(ceiling, Math.max(ATTEMPT_TARGET, spread));
    }
    var override = U.parseSizeInput(state.cfg.partCapBytes == null ? '' : String(state.cfg.partCapBytes));
    if (override) cap = Math.min(ceiling, Math.max(MIN_PART, override));
    return Math.max(MIN_PART, cap);
  }

  // What the transfer object weighs: the files, plus the container when there is
  // more than one. Every preview and every validation has to agree on this number,
  // or the promise made before sending differs from the one kept during it.
  function sourceSize(files) {
    var total = 0;
    (files || []).forEach(function (f) { total += Number(f.size) || 0; });
    if ((files || []).length > 1) total += MD.pack.archiveOverhead(files);
    return total;
  }
  ...
  function plan(files) {
    ...
    var total = sourceSize(files);
    var info = planInfoFor(backend, { size: total });
    return { backend: backend, cap: info.cap, total: total, parts: info.parts, count: info.parts.length,
             ...
             archive: files.length > 1 ? MD.pack.archiveName(files) : null };
  }
  ...
  function validateFiles(files) {
    ...
    if (b.noUpload) {
      ...
      if (files.length > 1) errs.push('A link you already have is one object: keep one file here, or send the rest as a second link. (Nothing is uploaded in this mode, so there is nothing for the page to archive.)');
      return errs;
    }
    if (!files.length) errs.push('Pick at least one file.');
    // Several files are allowed: they become one stored archive, because the link
    // carries one object. What is *not* allowed is pretending otherwise in
    // bring-your-own-link mode, where there is no upload to wrap anything into.
    var pi = planInfoFor(b, { size: sourceSize(files) });
    if (files.length && pi.linkFits === false) {
      var who = files.length > 1 ? files.length + ' files as ' + MD.pack.archiveName(files) : (files[0].name || 'this file');
      errs.push(who + ' would need a ' + pi.linkChars + '-character link at ' + U.fmtBytes(pi.cap) +
    ...
    var total = sourceSize(files);
    if (total === 0) errs.push(files.length > 1 ? 'Every one of those files is empty.' : 'The file is empty.');
  ...
  async function runSend() {
    ...
    // More than one file: the transfer object becomes a stored archive of them, and
    // building it reads every input once for the CRCs the format insists on having in
    // front of the data. That is seconds on a laptop and longer on a phone, so it is
    // said out loud while it happens — silence here looks exactly like a hang.
    if (files.length > 1) {
      A.busy = true;
      A.abort = new AbortController();
      UI.setBusy(true);
      UI.show('progress');
      UI.progress(0, 1, 'Indexing ' + files.length + ' files for the archive …');
      try {
        file = await MD.pack.makeArchive(files, {
          signal: A.abort.signal,
          onFile: function (i, n, nm) { UI.progress(i, n, 'Indexing file ' + (i + 1) + ' of ' + n + ' (for the archive index) …'); },
          onProgress: function (i, n, d, t) { UI.progress((i * 100) + (t ? (d / t) * 100 : 0), n * 100, 'Indexing file ' + (i + 1) + ' of ' + n + ' …'); }
        });
      } catch (e) {
        A.busy = false; UI.setBusy(false);
        if (e && e.cancelled) UI.note('Cancelled — nothing was uploaded, and nothing was left on the host.');
        else UI.error('The archive could not be built: ' + (e && e.message ? e.message : String(e)));
        return;
      }
      A.busy = false; UI.setBusy(false);
      UI.note(files.length + ' files travel as ' + file.name + ': stored in the archive, not compressed, so what the recipient unpacks is the bytes on this disk.');
    }
    var info = plan(files);
  ...
  async function afterFailure(backend, e, ids, expMs, ctx) {
    ...
    if (e && e.cancelled) UI.note('Cancelled.');
      ...
      // "Nothing reached the page" is a verdict about the network, not about the
      // file, and the two have different fixes — so the failed job says what is
      // still possible from this device instead of leaving the red box as the last
      // word. Only for status 0: a 403 or a 500 is a different conversation.
      if (e && (e.httpStatus === 0 || e.corsSuspected || (e.httpStatus === 403 && backend && backend.key === 'litterbox')) && UI.hostTrouble) UI.hostTrouble(backend, expMs);
    }
  ...
  function planInfoFor(backend, file) {
    var pw = (state.cfg.password || '').trim();
    var want = partCap(backend, file.size);
  ...
  function applyIncomingLink() {
    ...
    try {
      ...
      // A long hash that looks like a link should not silently fall back to the
      // Send tab. Say what failed once; the paste box has the same diagnosis.
      if (hash.length >= 40) UI.error((e && e.message ? e.message : String(e)) + ' — copy the complete URL or code again');
      return false;
    }
  }
