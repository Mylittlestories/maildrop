...
(function (global) {
  ...
  // A very old/local test link can be clipped by a mobile share or clipboard
  // exactly after the parts array's comma. Local mode is the only case where the
  // missing tail is safely reconstructable: its storage address is always the
  // fixed local://{id} template, and the part ids are generated by this page.
  // Never do this for a real host manifest; a clipped host token must be rejected.
  function repairLocalTail(json) {
    var t = String(json || '').replace(/\s+$/, '');
    if (t.slice(-1) !== ',') return null;
    var raw;
    try { raw = JSON.parse(t.slice(0, -1) + '}'); } catch (e) { return null; }
    if (!raw || raw.p !== 'local' || !Array.isArray(raw.p2) || !raw.p2.length) return null;
    for (var i = 0; i < raw.p2.length; i++) {
      if (!raw.p2[i] || !/^part-[A-Za-z0-9]+$/.test(String(raw.p2[i].i || ''))) return null;
    }
    raw.b = 'local://{id}';
    return raw;
  }
  ...
  function decode(token) {
    ...
    try {
      ...
      raw = repairLocalTail(json);
      if (!raw) {
        if (/,\s*$/.test(json)) throw new Error('manifest: incomplete download token — it ends halfway through the manifest; copy the entire link or code again');
        throw new Error('manifest: corrupt download token (the code is not complete JSON)');
      }
    }
  ...
  function findManifest(text) {
    ...
    // Do not call an apparently valid-but-clipped code "no token": that sends
    // somebody back to copy the same broken text. Keep the old message for prose
    // and other ordinary garbage, which is much more common in this box.
    var bareCode = /^[A-Z2-7\s_=\-]+$/i.test(s) && s.replace(/[^A-Z2-7]/gi, '').length >= 40;
    if (sawCandidate && lastError && (s.indexOf('#') >= 0 || bareCode)) {
      throw new Error(lastError.message + ' — copy the complete URL or code again');
    }
