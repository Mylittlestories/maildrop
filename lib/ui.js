...
(function (global) {
  ...
  function corsJsonForOrigin(origin){
    var o=String(origin||'').trim()||'https://mylittlestories.github.io';
  ...
  function renderCors(){
    ...
    if(!ta) return;
    var origin='';
    try{ origin=(global.location&&global.location.origin)||'https://mylittlestories.github.io'; }catch(e){}
    ta.value=corsJsonForOrigin(origin);
  }
  UI.renderCors=renderCors;
  function collectDroppedFiles(dt, cb){
    var out=[];
    try{
      var items = dt.items ? Array.prototype.slice.call(dt.items) : [];
      var hasEntry = items.length && items[0] && typeof items[0].webkitGetAsEntry === 'function';
      if(!hasEntry || !items[0].webkitGetAsEntry()) { cb(Array.prototype.slice.call(dt.files||[])); return; }
      var entries = items.map(function(it){ return it.webkitGetAsEntry(); }).filter(Boolean);
      var pending = entries.length;
      if(!pending){ cb([]); return; }
      function done(){ if(--pending===0) cb(out); }
      entries.forEach(function(entry){ traverse(entry, '', out, done); });
    }catch(e){ cb(Array.prototype.slice.call(dt.files||[])); }
  }
  function traverse(entry, prefix, out, done){
    if(entry.isFile){
      entry.file(function(file){
        try{ Object.defineProperty(file,'relativePath',{value: prefix + file.name, writable:true}); }catch(_){ file.relativePath = prefix + file.name; }
        out.push(file); done();
      }, function(){ done(); });
      ...
      function readAll(){
        reader.readEntries(function(batch){
          ...
          function subDone(){ if(--subPending===0) readAll(); }
          batch.forEach(function(e){ traverse(e, dirPath, out, subDone); });
  ...
  function renderFiles() {
    ...
    box.innerHTML = pre + f.map(function (x, i) {
      ...
      var disp = esc(x.relativePath || x.webkitRelativePath || x.name);
      var extra = (x.relativePath||x.webkitRelativePath) ? ' <span class="mini">(' + esc(x.name) + ')</span>' : '';
      return '<div class="fl"><div><div class="nm">' + disp + extra + '</div>' +
  ...
  UI.showLink = function (link, m, token) {
    ...
    writeMailBody();
    var localOnly = m.p === 'local';
    id('linkNote').className = localOnly ? 'note warn' : 'note ok';
    id('linkNote').textContent = localOnly
      ? 'Offline test complete. The parts never left this browser, so this is not a shareable link — use “Open it on this device”, or choose Litterbox / your own bucket before emailing someone.'
      : 'Link ready. Paste it into any mail client (Gmail, Outlook, Apple Mail) and send — or use the button above. ' +
        (m.d ? 'The host deletes the parts after ' + U.fmtDuration(m.d / 3600000) + '.' : '');
    // A local manifest is useful for the in-tab preview, but every share button
    // would create a recipient-facing link that cannot work on another browser.
    ['btnCopyLink','btnCopyCode','btnMailto','btnGmail','btnOutlook','btnWhatsApp','btnTelegram','btnQrShow'].forEach(function (k) {
      if (id(k)) id(k).hidden = localOnly;
    });
    if (id('btnShare')) id('btnShare').hidden = localOnly || !navigator.share;
    if (localOnly && id('linkBackup')) id('linkBackup').hidden = true;
    try{
      if(MD.app._linkTimer) clearInterval(MD.app._linkTimer);
      if(m.d && m.x){
        var noteEl=id('linkNote');
        function updLinkNote(){
          var left = m.x - Date.now();
          if(left<=0){ noteEl.textContent += ' \u2014 now expired.'; clearInterval(MD.app._linkTimer); return; }
          noteEl.textContent = 'Link ready. Paste it into any mail client and send \u2014 or use the button above. ' +
            (m.d ? 'The host deletes the parts after ' + U.fmtDuration(m.d / 3600000) + ' (expires '+U.fmtDuration(left/3600000)+' from now).' : '');
        }
        MD.app._linkTimer=setInterval(function(){ if(!id('linkCard').hidden) updLinkNote(); }, 60000);
      }
    }catch(_){}
    if (navigator.share && !localOnly) id('btnShare').hidden = false;
  };
  ...
  UI.startReceive = function (m) {
    ...
    if (missing.length) {
      ...
      id('recvNote').className = 'note warn';
      id('recvNote').textContent = 'This is an offline test link. Its parts exist only in the browser that created it; for another person, send again with Litterbox or your own bucket.';
    } else {
      id('recvNote').className = 'note';
  ...
  function wire() {
    ...
    drop.addEventListener('drop', function (e) {
      ...
      if(!dt) return;
      collectDroppedFiles(dt, function(add){
        if(!add.length) return;
        var cur = MD.app.state.files || [];
        var seen = {};
        cur.forEach(function (f) { seen[(f.relativePath||f.webkitRelativePath||f.name) + '|' + f.size + '|' + (f.lastModified || 0)] = 1; });
        add.forEach(function (f) {
          var k = (f.relativePath||f.webkitRelativePath||f.name) + '|' + f.size + '|' + (f.lastModified || 0);
          if (!seen[k]) { cur.push(f); seen[k] = 1; }
        });
        MD.app.state.files = cur;
        renderFiles();
        if(add.some(function(f){ return (f.relativePath||f.webkitRelativePath||'').indexOf('/')>=0; })){
          UI.note(add.length+' file(s) from folder \u2014 kept inside the zip with their paths.');
        }
      });
    });
    ...
    var bw=id('btnWhatsApp');
    if(bw) bw.onclick=function(){
      var link=id('linkText').value||'';
      if(!link){ UI.note('Make the link first.'); return; }
      var body=id('bodyText').value||link;
      var txt=body.length>1200?link:body;
      var url='https://wa.me/?text='+encodeURIComponent(txt);
      var w=null; try{ w=global.open(url,'_blank','noopener'); }catch(e){}
      UI.note(w?'WhatsApp opened — pick a chat and send.':'Pop-up blocked. Allow pop-ups or copy the link.');
    };
    var bt=id('btnTelegram');
    if(bt) bt.onclick=function(){
      var link=id('linkText').value||'';
      if(!link){ UI.note('Make the link first.'); return; }
      var subj=id('subjInput').value||'';
      var url='https://t.me/share/url?url='+encodeURIComponent(link)+'&text='+encodeURIComponent(subj);
      var w=null; try{ w=global.open(url,'_blank','noopener'); }catch(e){}
      UI.note(w?'Telegram opened — pick a chat and send.':'Pop-up blocked. Copy the link instead.');
    };
    // QR code: rendered with the vendored qrcode.js (davidshimjs, MIT). No network, no data leaves the page.
    var bq=id('btnQrShow'), bh=id('btnQrHide'), qb=id('qrBox'), qc=id('qrCanvas');
    function showQr(){
      var link=id('linkText').value||'';
      if(!link){ UI.note('Make the link first.'); return; }
      if(!qb||!qc) return;
      qc.innerHTML='';
      try{
        // qrcode.js expects a string and renders a table/canvas inside the element
        new QRCode(qc,{text:link,width:220,height:220,colorDark:'#000000',colorLight:'#ffffff',correctLevel:QRCode.CorrectLevel.M});
      }catch(e){
        qc.textContent='QR too large for this link ('+e.message+'). Send the short backup links instead — they are in the folded section below.';
      }
      qb.hidden=false;
      qc.scrollIntoView&&qc.scrollIntoView({behavior:'smooth',block:'center'});
    }
    if(bq) bq.onclick=showQr;
    if(bh) bh.onclick=function(){ if(qb) qb.hidden=true; };
    // keep QR in sync if the link changes (e.g. password toggled)
    var _origShowLink=UI.showLink;
    UI.showLink=function(link,m,token){
      _origShowLink(link,m,token);
      if(qb&&!qb.hidden) showQr();
    };
    ...
    // Do not auto-submit on paste. The page says “paste, then Open it”; doing both
    // here and on the button click produced two identical red errors on mobile and
    // made a clipped code look like two separate failures.
  ...
  function bind(settingKey, elId, transform, after) {
    ...
    var cc=id('btnCopyCors');
    if(cc) cc.onclick=function(){ MD.email.copyText(id('corsJson').value).then(function(ok){ id('corsHint').textContent=ok?'CORS JSON copied — paste it into your bucket.':'Copy blocked — select it manually.'; }); };
    renderCors();
    ...
    (function(){
      var supported = MD.p2p && MD.p2p.supported && MD.p2p.supported();
      var card=id('p2pCard'), noSup=id('p2pNoSupport'), steps=id('p2pSteps');
      var btnOpen=id('btnOpenP2p'), btnCreate=id('btnP2pCreate'), taOffer=id('p2pOffer'), taAnswer=id('p2pAnswer');
      var btnCopyOffer=id('btnP2pCopyOffer'), btnQrOffer=id('btnP2pQrOffer'), qrBox=id('p2pQrBox'), qrCanvas=id('p2pQrCanvas'), btnCloseOffer=id('btnP2pCloseOffer');
      var btnConnect=id('btnP2pConnect'), btnCancel=id('btnP2pCancel'), bar=id('p2pBar'), msg=id('p2pMsg'), stat=id('p2pStatus'), shareRow=id('p2pShareRow');
      var btnShareWa=id('btnP2pShareOffer'), btnShareTg=id('btnP2pShareOfferTg');
      var curPc=null, curFile=null;
      function setSupport(){
        if(!supported){ if(noSup) noSup.hidden=false; if(steps) steps.hidden=true; if(btnCreate) btnCreate.disabled=true; return false; }
        if(noSup) noSup.hidden=true; return true;
      }
      setSupport();
      if(btnOpen) btnOpen.onclick=function(){
        if(!MD.app.state.files.length){ UI.error('Pick a file (or folder) first, then press this.'); return; }
        if(!setSupport()) return;
        card.hidden=false;
        card.scrollIntoView({behavior:'smooth', block:'start'});
        stat.textContent='Ready to create a code.';
      };
      if(btnCloseOffer) btnCloseOffer.onclick=function(){ taOffer.value=''; taAnswer.value=''; if(qrBox) qrBox.hidden=true; msg.textContent=''; stat.textContent=''; bar.style.width='0%'; if(shareRow) shareRow.hidden=true; };
      if(btnCancel) btnCancel.onclick=function(){
        if(curPc) MD.p2p.close(curPc);
        curPc=null; card.hidden=true; taOffer.value=''; taAnswer.value=''; msg.textContent='Cancelled.'; bar.style.width='0%';
      };
      if(btnCreate) btnCreate.onclick=async function(){
        if(!MD.app.state.files.length){ UI.error('Pick a file first.'); return; }
        btnCreate.disabled=true; stat.textContent='Creating code...'; msg.textContent='';
        try{
          var res=await MD.p2p.createOffer(MD.app.state.files);
          curPc=res.pc; curFile=res.file;
          taOffer.value=res.code;
          stat.textContent='Code ready \u2014 send it to them. ('+res.code.length+' chars)';
          msg.textContent='Keep this tab open. They paste the code on their Receive tab, send you an answer, you paste it below.';
          if(shareRow) shareRow.hidden=false;
          if(qrBox && qrCanvas){
            try{ qrCanvas.innerHTML=''; new QRCode(qrCanvas,{text:res.code,width:200,height:200,colorDark:'#000000',colorLight:'#ffffff',correctLevel:QRCode.CorrectLevel.M}); qrBox.hidden=false; }catch(e){}
          }
          btnCreate.disabled=false;
        }catch(e){ stat.textContent=''; msg.textContent=e.message; btnCreate.disabled=false; }
      };
      if(btnCopyOffer) btnCopyOffer.onclick=function(){ MD.email.copyText(taOffer.value).then(function(ok){ UI.note(ok?'Code copied.':'Copy blocked.'); }); };
      if(btnQrOffer) btnQrOffer.onclick=function(){
        var c=taOffer.value.trim(); if(!c){ UI.note('Create a code first.'); return; }
        if(qrBox) qrBox.hidden=!qrBox.hidden;
        if(!qrBox.hidden){ try{ qrCanvas.innerHTML=''; new QRCode(qrCanvas,{text:c,width:200,height:200,colorDark:'#000000',colorLight:'#ffffff',correctLevel:QRCode.CorrectLevel.M}); }catch(e){ qrCanvas.textContent=e.message; } }
      };
      if(btnConnect) btnConnect.onclick=async function(){
        var ans=taAnswer.value.trim();
        if(!ans){ UI.error('Paste their answer code first.'); return; }
        if(!curPc){ UI.error('Create a code first.'); return; }
        btnConnect.disabled=true; stat.textContent='Connecting...'; bar.style.width='0%';
        try{
          await MD.p2p.connectWithAnswer(curPc, ans, function(done, total){
            var f=Math.max(0,Math.min(100, total? done/total*100:0));
            bar.style.width=f.toFixed(1)+'%';
            msg.textContent='Sending '+U.fmtBytes(done)+' / '+U.fmtBytes(total)+' ('+f.toFixed(0)+'%) \u2014 keep the tab open';
          });
          bar.style.width='100%'; msg.textContent='Sent \u2713 They got '+U.fmtBytes(curFile.size)+' ('+curFile.name+'). You can close this card.';
          stat.textContent='Done.';
          UI.note('Direct transfer complete \u2014 '+U.fmtBytes(curFile.size)+' sent without a host.');
        }catch(e){ msg.textContent=e.message; stat.textContent='Failed.'; }
        btnConnect.disabled=false;
      };
      if(btnShareWa) btnShareWa.onclick=function(){
        var c=taOffer.value.trim(); if(!c){ UI.note('Create a code first.'); return; }
        var url='https://wa.me/?text='+encodeURIComponent('MailDrop direct code \u2014 paste in Receive -> Join:\n\n'+c);
        var w=null; try{ w=global.open(url,'_blank','noopener'); }catch(_){}
        UI.note(w?'WhatsApp opened.':'Pop-up blocked \u2014 copy the code.');
      };
      if(btnShareTg) btnShareTg.onclick=function(){
        var c=taOffer.value.trim(); if(!c){ UI.note('Create a code first.'); return; }
        var url='https://t.me/share/url?url='+encodeURIComponent((typeof MD!== 'undefined' && MD.app && MD.app.receiveBaseUrl ? MD.app.receiveBaseUrl() : (global.location && global.location.href ? global.location.href.split('#')[0] : 'https://example.com/')))+'&text='+encodeURIComponent('MailDrop direct code:\n'+c);
        var w=null; try{ w=global.open(url,'_blank','noopener'); }catch(_){}
        UI.note(w?'Telegram opened.':'Pop-up blocked.');
      };
      var rNoSup=id('p2pRecvNoSupport'), rOffer=id('p2pRecvOffer'), rAnswer=id('p2pRecvAnswer'), rCreate=id('btnP2pRecvCreate');
      var rCopy=id('btnP2pCopyAnswer'), rQrBtn=id('btnP2pQrAnswer'), rQrBox=id('p2pRecvQrBox'), rQrCanvas=id('p2pRecvQrCanvas');
      var rBar=id('p2pRecvBar'), rMsg=id('p2pRecvMsg'), rStat=id('p2pRecvStatus'), rShareRow=id('p2pRecvShareRow');
      var rShareWa=id('btnP2pShareAnswer'), rShareTg=id('btnP2pShareAnswerTg');
      var recvPc=null;
      if(rNoSup && !supported) rNoSup.hidden=false;
      if(rCreate) rCreate.onclick=async function(){
        var offer=rOffer.value.trim();
        if(!offer){ UI.error('Paste their code first.'); return; }
        rCreate.disabled=true; rStat.textContent='Creating answer...'; rMsg.textContent='';
        try{
          var res=await MD.p2p.createAnswer(offer, function(file){
            rMsg.textContent='Got '+file.name+' ('+U.fmtBytes(file.size)+') \u2014 saving...';
            MD.receive.saveBlob(file, file.name).then(function(){ rMsg.textContent='Saved '+file.name+' \u2713'; UI.note('Direct transfer received \u2014 saved.'); });
            rBar.style.width='100%';
          }, function(done,total){
            var f=Math.max(0,Math.min(100,total?done/total*100:0));
            rBar.style.width=f.toFixed(1)+'%';
            rMsg.textContent='Receiving '+U.fmtBytes(done)+' / '+(total?U.fmtBytes(total):'?')+' ('+f.toFixed(0)+'%)';
          });
          recvPc=res.pc;
          rAnswer.value=res.code;
          rStat.textContent='Answer ready \u2014 send it back to them ('+res.code.length+' chars). Waiting for them to connect...';
          if(rShareRow) rShareRow.hidden=false;
          res.done.then(function(o){ }, function(e){ rMsg.textContent=e.message; rStat.textContent='Failed.'; });
          if(rQrBox && rQrCanvas){
            try{ rQrBox.hidden=false; rQrCanvas.innerHTML=''; new QRCode(rQrCanvas,{text:res.code,width:200,height:200,colorDark:'#000000',colorLight:'#ffffff',correctLevel:QRCode.CorrectLevel.M}); }catch(e){}
          }
        }catch(e){ rMsg.textContent=e.message; rStat.textContent=''; }
        rCreate.disabled=false;
      };
      if(rCopy) rCopy.onclick=function(){ MD.email.copyText(rAnswer.value).then(function(ok){ UI.note(ok?'Answer copied.':'Copy blocked.'); }); };
      if(rQrBtn) rQrBtn.onclick=function(){
        var c=rAnswer.value.trim(); if(!c){ UI.note('Create an answer first.'); return; }
        if(rQrBox) rQrBox.hidden=!rQrBox.hidden;
        if(!rQrBox.hidden){ try{ rQrCanvas.innerHTML=''; new QRCode(rQrCanvas,{text:c,width:200,height:200,colorDark:'#000000',colorLight:'#ffffff',correctLevel:QRCode.CorrectLevel.M}); }catch(e){ rQrCanvas.textContent=e.message; } }
      };
      if(rShareWa) rShareWa.onclick=function(){
        var c=rAnswer.value.trim(); if(!c){ UI.note('Create an answer first.'); return; }
        var url='https://wa.me/?text='+encodeURIComponent('MailDrop direct answer \u2014 paste it back:\n\n'+c);
        var w=null; try{ w=global.open(url,'_blank','noopener'); }catch(_){}
        UI.note(w?'WhatsApp opened.':'Pop-up blocked.');
      };
      if(rShareTg) rShareTg.onclick=function(){
        var c=rAnswer.value.trim(); if(!c){ UI.note('Create an answer first.'); return; }
        var url='https://t.me/share/url?url='+encodeURIComponent((typeof MD!== 'undefined' && MD.app && MD.app.receiveBaseUrl ? MD.app.receiveBaseUrl() : (global.location && global.location.href ? global.location.href.split('#')[0] : 'https://example.com/')))+'&text='+encodeURIComponent('MailDrop direct answer:\n'+c);
        var w=null; try{ w=global.open(url,'_blank','noopener'); }catch(_){}
        UI.note(w?'Telegram opened.':'Pop-up blocked.');
      };
    })();
  ...
  var histTimer=null;
  function fmtLeft(at,exp){
    var nExp = Number(exp)||0;
    if(!nExp) return '';
    var left = (Number(at)||0) + nExp - Date.now();
  ...
  function renderHistory() {
    ...
    var box = id('histList');
    if (!h.length) { box.textContent = 'nothing yet'; if(histTimer){ clearInterval(histTimer); histTimer=null; } return; }
    box.innerHTML = h.map(function (e, i) {
      var expiry = null; var prov='';
      try{ var mm=MD.manifest.decode((e.u.split('#')[1]||'').split('&')[0]); expiry=mm.d; prov=mm.p||''; }catch(_){}
      var left = expiry ? fmtLeft(e.at, expiry) : '';
      var leftCls = left==='expired' ? 'pill bad' : (left && left.indexOf('m left')>=0 ? 'pill warn' : 'pill');
      return '<div class="fl" style="padding:8px 10px;gap:8px"><div class="grow" style="min-width:0"><div class="nm" style="font-size:14px;word-break:break-all">' + esc(e.n) + '</div>' +
        '<div class="mt">' + U.fmtBytes(e.z) + ' \u00b7 ' + new Date(e.at).toLocaleString() + (prov?' \u00b7 '+esc(prov):'') + '</div>' +
        (left? '<span class="'+leftCls+'" style="margin-top:4px">'+esc(left)+'</span>':'') +
        '</div>' +
        '<button class="btn small ghost" data-re="' + i + '" data-act="copy">copy</button>' +
        '<button class="btn small ghost" data-re="' + i + '" data-act="open">open</button>' +
        '<button class="btn small ghost" data-re="' + i + '" data-act="del" title="Remove from this browser">\u2715</button></div>';
    }).join('');
    box.querySelectorAll('[data-re]').forEach(function (b) {
      b.onclick = function () {
        var idx = Number(b.getAttribute('data-re').trim());
        var e = h[idx];
        if (!e) return;
        var act=b.getAttribute('data-act');
        if (act === 'copy') MD.email.copyText(e.u).then(function (ok) { UI.note(ok?'Link copied.':'Copy blocked.'); });
        else if (act === 'open') { tab('receive'); try{ UI.startReceive(MD.email.extract(e.u)); }catch(err){ UI.error(err.message); } }
        else if (act === 'del'){
          var cur = MD.app.history().slice(); cur.splice(idx,1);
          try{ global.localStorage.setItem('maildrop.history.v1', JSON.stringify(cur)); }catch(_){}
          UI.note('Removed from history (the host still deletes on its own schedule).');
          renderHistory();
        }
      };
    });
    if(!histTimer){
      histTimer=setInterval(function(){
        if(!document.hidden && MD.app.history().length) renderHistory();
      }, 60000);
    }
  }
  ...
  UI.init = function () {
    ...
    try{ if(typeof navigator!=='undefined' && 'serviceWorker' in navigator){ navigator.serviceWorker.register('./sw.js').catch(function(){}); } }catch(e){}
    // If the default provider is known to be flaky from this network, say so before the user waits for an upload to fail.
    try{
      if(MD.app.state.cfg.backend==='litterbox' && MD.backends.probeHost){
        var hintEl=id('probeHint');
        MD.backends.probeHost('litterbox', MD.app.state.cfg.s3).then(function(r){
          if(!r.ok){
            UI.warn('Litterbox looks blocked from this network right now — the “Check this host” panel below has the details. Your own bucket (Settings) or a different network will work.');
            if(hintEl) hintEl.textContent='looks blocked from this network — tap Check for details';
          }
        },function(){});
      }
    }catch(e){}
