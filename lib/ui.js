...
(function (global) {
  ...
  function id(x) { return els[x] || (els[x] = $(x)); }
  ...
  function banner(msg, kind) {
    ...
    d.textContent = msg;
    ctx.appendChild(d);
    setTimeout(function () { if (d.parentNode) d.parentNode.removeChild(d); }, kind === 'bad' ? 14000 : 6000);
  }
  ...
  function tab(name) {
    ['send', 'receive', 'settings'].forEach(function (n) {
      ...
      var b = document.querySelector('nav button[data-tab=' + n + ']');
  ...
  function renderFiles() {
    ...
    var box = id('fileList');
    if (!f.length) { box.innerHTML = ''; id('btnStart').disabled = true; renderPlan(); return; }
    box.innerHTML = f.map(function (x, i) {
      return '<div class="fl"><div><div class="nm">' + esc(x.name) + '</div>' +
        '<div class="mt">' + U.fmtBytes(x.size) + ' · ' + esc(x.type || 'unknown type') + ' · modified ' + new Date(x.lastModified).toISOString().slice(0, 10) + '</div></div>' +
  ...
  function renderPlan() {
    ...
    if (cap < MD.app.SOFT_MIN_PART && !errs.length) {
      html += '<div class="note">You set a part size below the recommended ' + U.fmtBytes(MD.app.SOFT_MIN_PART) + ' — fine for testing, wasteful for real sends.</div>';
    }
    if (n > 1) html += '<div class="note warn"><b>' + U.fmtBytes(total) + ' does not fit one upload.</b> MailDrop splits it into ' + n +
      ' parts, uploads them one after another, and still gives you a <b>single link</b> — the receiving page stitches them back. If a part fails, only that part retries.</div>';
    if (b.key !== 'selfhost') {
      html += '<div class="note">Public hosts without accounts are rate-limited and can vanish. If the send fails with a CORS/4xx error, switch provider or set up your own bucket in Settings. ' +
        'Recipients need nothing: they click the link and save.</div>';
    }
    if (errs.length) html += errs.map(function (e) { return '<div class="note bad">' + esc(e) + '</div>'; }).join('');
    box.innerHTML = html;
    id('sendHint').textContent = errs.length ? 'fix the problems above' : (n + ' part' + (n === 1 ? '' : 's') + ' · ' + U.fmtBytes(total) + ' → 1 link');
  ...
  UI.offerDirect = function (url, why) {
    ...
    row.hidden = false;
    id('recvDirect').setAttribute('href', url);
  ...
  function wire() {
    ...
    fi.onchange = function () { MD.app.state.files = Array.prototype.slice.call(fi.files || []); renderFiles(); };
    ...
    drop.addEventListener('drop', function (e) {
      ...
      if (items && items.length) { MD.app.state.files = Array.prototype.slice.call(items); renderFiles(); }
    });
    ...
    id('btnStart').textContent = 'Start transfer';
    id('btnCancel').onclick = function () { MD.app.cancel(); };
    ...
    id('btnOpenLink').onclick = function () { global.open(id('linkText').value, '_blank', 'noopener'); };
    id('btnShare').onclick = function () { MD.email.share(id('linkText').value); };
    id('btnMailto').onclick = function () {
      var ok = MD.email.openInClient(id('toInput').value.trim(), id('subjInput').value, id('bodyText').value);
      UI.note(ok ? 'Handed to your mail client — check that the message arrived complete, then send.' : 'No mail client responded. Copy the message instead.');
    };
    ...
    id('btnWipe').onclick = function () {
      ...
      try { global.localStorage.removeItem('maildrop.settings.v1'); global.localStorage.removeItem('maildrop.history.v1'); } catch (e) { }
    ...
    MD.backends.s3.presignPutUrl(cfg, key, 1, {}).then(function (signed) {
      ...
      return MD.backends.xhrUpload(signed.url, new Blob([new Uint8Array(1)], { type: 'application/octet-stream' }), { method: 'PUT' })
        ...
        .catch(function (e) {
          ...
          lines.push('', 'Most likely the bucket has no CORS rule allowing PUT from this origin — see docs/SETUP-BUCKET.md.');
          out.textContent = lines.join('\n');
