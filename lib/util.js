...
(function (global) {
  ...
  function cleanName(name) {
    // slashes and control characters are a path problem; < > " ' ` are a markup
    // problem — a name also arrives inside someone else's link, so it must not be
    // able to look like HTML when it is put on screen.
  ...
  // Anything that comes out of a manifest is attacker-shaped: a mail client can
  // deliver a link whose part URL, host URL or file name says something the page
  // never should. One gate, checked where the manifest is read, so no consumer
  // has to remember. local:// stays allowed for the in-page test backend.
  function safeUrl(u, allowLocal) {
    var s = String(u == null ? '' : u).trim();
    if (!s || s.length > 4096) return '';
    if (/[\u0000-\u0020\u007f"'<>`\\]/.test(s)) return '';
    if (/^local:\/\/[\w.-]+$/i.test(s)) return allowLocal ? s : '';
    if (!/^https?:\/\/[^\s/?#]+[^\s]*$/i.test(s)) return '';
    var proto = /^([a-z]+):/i.exec(s)[1].toLowerCase();
