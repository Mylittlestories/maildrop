...
(function (global) {
  ...
  // A pasted "To:" may contain nothing but addresses. A stray ?, #, quote or
  // newline would otherwise extend the mailto: URL — or quietly add recipients
  // the sender never meant to include — so each one is checked, not encoded.
  function safeAddress(list) {
    var kept = [], dropped = [];
    String(list == null ? '' : list).split(/[,;\s]+/).forEach(function (a) {
      if (!a) return;
      if (/^[A-Za-z0-9._%+!#$&*'/=?^`{}~]{1,64}@[A-Za-z0-9.-]+\.[A-Za-z]{2,}$/.test(a) && a.length <= 254) kept.push(a);
      else dropped.push(a);
    });
    return { to: kept.join(','), dropped: dropped, ok: kept.length > 0 };
  }

  function mailtoUrl(address, subject, body) {
    var s = U.truncateUtf8(subject || '', 300);
    var b = body || '';
    var to = safeAddress(address);
    var prefix = 'mailto:' + to.to + '?subject=' + encodeURIComponent(s) + '&body=';
  ...
  // Webmail compose links, for the very common case of a browser with no mail
  // app behind mailto: (a phone, or a laptop that only ever uses webmail). These are
  // *navigations the user starts by clicking*, not resources the page loads: nothing
  // here is fetched, embedded or contacted before the click, which is what the
  // offline-and-no-third-party test in tests/page.test.js keeps honest.
  //
  // The address is put through the same filter as mailto: — an address with a stray
  // ?, # or quote could otherwise add a recipient or extend the query string.
  var COMPOSERS = {
    gmail: function (q) {
      return 'https://mail.google.com/mail/?view=cm&fs=1' + (q.to ? '&to=' + q.to : '') + '&su=' + q.su + '&body=' + q.body;
    },
    outlook: function (q) {
      return 'https://outlook.office.com/mail/deeplink/compose?' + (q.to ? 'to=' + q.to + '&' : '') + 'subject=' + q.su + '&body=' + q.body;
    }
    // Another provider? MD.config.composeUrl wins over the two above. It is a
    // template with {to} {subject} {body} placeholders and must be https://.
  };

  function composeUrl(provider, address, subject, body) {
    var to = safeAddress(address);
    var q = {
      to: encodeURIComponent(to.to),
      su: encodeURIComponent(subject || ''),
      body: encodeURIComponent(body || '')
    };
    var custom = (MD.config && MD.config.composeUrl) || '';
    var url;
    if (custom) {
      url = String(custom).replace(/\{to\}/g, q.to).replace(/\{subject\}/g, q.su).replace(/\{body\}/g, q.body);
    } else {
      var make = COMPOSERS[provider] || COMPOSERS.gmail;
      url = make(q);
    }
    if (!/^https:\/\//i.test(url)) throw new Error('The compose URL has to be https:// — refusing to open ' + U.truncateUtf8(String(url).slice(0, 40), 40));
    return { url: url, hasTo: to.ok, dropped: to.dropped };
  }
