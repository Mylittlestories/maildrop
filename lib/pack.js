...
(function (global) {
  ...
  function zipArchiveName(files) {
    var base = String((files[0] && files[0].name) || 'files');
  ...
  function zipBuild(files, crcs, zip64) {
    ...
    for (i = 0; i < files.length; i++) {
      ...
      var nm = zipUnique(used, zipSafeName(f.name, i));
  ...
  function archiveOverhead(files) {
    ...
    var lay = zipLayout(fs.map(function (f) { return { name: f.name, size: Number(f.size) || 0, lastModified: f.lastModified }; }), crcs);
    var bodies = 0;
  ...
  async function makeArchive(files, opts) {
    ...
    var view = fs.map(function (f) { return { name: f.name, size: Number(f.size) || 0, lastModified: f.lastModified }; });
    var crcs = [];
