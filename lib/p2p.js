...
(function(global){
  ...
  function waitGather(pc){
    return new Promise(function(res){
      ...
      // This code sends one self-contained SDP rather than trickling candidates,
      // so give slow mobile DNS/STUN paths time to finish. 1.5 seconds regularly
      // produced an offer before the srflx candidate had arrived.
  ...
  function newPC(){
    // Multiple STUN paths help when one provider is slow or filtered. STUN can
    // discover a route; it cannot relay traffic. A symmetric NAT or a carrier
    // firewall still needs TURN, which this static/no-server app deliberately does
    // not operate.
    var cfg = {iceServers:[
  ...
  P2P.createOffer = async function(files){
    ...
    if(files && files.length > 1){
      if(!MD.pack || !MD.pack.makeArchive) throw new Error('This build cannot pack several files for a direct transfer.');
      try{
        var made = await MD.pack.makeArchive(files);
        fileForChannel = made;
        archive = true;
      }catch(e){
        throw new Error('Could not build the direct-transfer archive: ' + (e && e.message ? e.message : String(e)));
      }
    } else {
  ...
  P2P.connectWithAnswer = async function(pc, answerCode, onProgress){
    ...
    // Wait for the data channel and only resolve after the EOF marker has been
    // sent. On fast/same-LAN connections the channel can open before the event
    // handler below is installed; checking readyState closes that race.
    return new Promise(function(resolve, reject){
      var settled = false, started = false, connected = false;
      var to = setTimeout(function(){
        fail(new Error('connection timed out — the answer may be wrong or these networks are blocking WebRTC. Try both devices on the same Wi‑Fi, or use the host upload.'));
      }, 30000);
      function finish(){
        if(settled) return;
        settled = true;
      ...
      function fail(e){
        ...
        settled = true;
        clearTimeout(to);
        reject(e instanceof Error ? e : new Error(String(e || 'connection failed')));
      }
      async function start(){
        if(started || settled || !pc._md_dc || pc._md_dc.readyState !== 'open') return;
        started = true;
        try{ await sendFileOver(pc._md_dc, pc._md_file, onProgress); finish(); }
        catch(e){ fail(e); }
      }
      function state(){
        if(pc.connectionState === 'connected') connected = true;
        if(pc.connectionState === 'failed' || pc.connectionState === 'closed') {
          fail(new Error('connection failed — these networks did not expose a direct route. A TURN relay would be needed for this carrier/NAT, and MailDrop does not run one. Try both devices on the same Wi‑Fi, or use the host upload.'));
          return;
        }
        if(connected) start();
      }
      ...
      pc.addEventListener('iceconnectionstatechange', function(){
        if(pc.iceConnectionState === 'failed') fail(new Error('ICE failed — these networks need a TURN relay for WebRTC. Try both devices on the same Wi‑Fi, or use the host upload.'));
        else state();
      });
      if(pc._md_dc){
        pc._md_dc.onopen = start;
        pc._md_dc.onerror = function(e){ fail(new Error('channel error: '+(e && e.message || 'unknown'))); };
        pc._md_dc.onclose = function(){ if(!settled) fail(new Error('channel closed before the file finished sending.')); };
        if(pc._md_dc.readyState === 'open') start();
      }
      // A state transition can happen before the listeners are attached.
      state();
    });
  };
  ...
  P2P.createAnswer = async function(offerCode, onFile, onProgress){
    ...
    function answerFailed(){
      doneReject(new Error('connection failed — the two networks need a TURN relay or are blocking WebRTC. Try both devices on the same Wi‑Fi, or use the host upload.'));
    }
    pc.addEventListener('connectionstatechange', function(){
      if(pc.connectionState === 'failed') answerFailed();
    });
    pc.addEventListener('iceconnectionstatechange', function(){
      if(pc.iceConnectionState === 'failed') answerFailed();
    });
