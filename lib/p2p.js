...
(function(global){
  ...
  P2P.createOffer = async function(files){
    ...
    // reuse the same archive the normal flow would send
    if(files && files.length > 1){
      // MD.pack.makeArchive builds a stored zip via MD.pack.zipBuild; it needs to be async because it may be large
      // but we have no async zip here in pack — use makeArchive which returns {file}
      // Fallback: if pack not loaded, just send first file
      try{
        if(MD.pack && MD.pack.makeArchive){
          var made = MD.pack.makeArchive(files);
          // makeArchive may return a File/Blob synchronously
          fileForChannel = made && made.file ? made.file : made;
          archive = true;
        }
