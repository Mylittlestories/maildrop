...
(function(global){
  ...
  P2P.connectWithAnswer = async function(pc, answerCode, onProgress){
    ...
    return new Promise(function(resolve, reject){
      ...
      function failed(){
        clearTimeout(to);
        reject(new Error('connection failed — these networks did not expose a direct route. A TURN relay would be needed for this carrier/NAT, and MailDrop does not run one. Try both devices on the same Wi‑Fi, or use the host upload.'));
      }
      pc.addEventListener('connectionstatechange', function(){
        if(pc.connectionState === 'connected'){ clearTimeout(to); resolve(); }
        else if(pc.connectionState === 'failed'){ failed(); }
      });
